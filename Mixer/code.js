var startTime =0;
var dest =  null;
window.audioContext = null, window.MediaRecorder = null, window.gainNode = null, window.analyser = null, window.frequencyDataArray = null, window.stopSounds = new List();
try {
...
//window.mediaRecorder = new MediaRecorder(dest.stream, options);
}
catch(e) {
...
SpriteMorph.prototype.initBlocks = function () {
    SpriteMorph.prototype.blocks = {
        ...
        doPlayNote: {
            type: 'command',
            category: 'sound',
            spec: 'play frequency %n at time %n for %n secs',
            defaults: [200, 0,0.5]
        },
        doChangeTempo: {
            type: 'command',
...
SpriteMorph.prototype.playSoundTime = function (input, time) {
	sound = detect(
		this.sounds.asArray(),
...
function str2ArrayBuffer(str,callback)
{
    ...
    req.onload = function fileLoaded(e){ callback(e.target.response); };
    req.send();
}
...
Process.prototype.doPlayNote = function (pitch, time, duration) {
  ...
  oscillator[oscillator.start ? 'start' : 'noteOn'](time+startTime);
  oscillator.stop(time+startTime+duration);
  stopSounds.add(oscillator);
};
...
SpriteMorph.prototype.positionTalkBubble = function () {
    ...
    if (bubble.isLabel) {
        bubble.setCenter(new Point(this.center().x+this.scale*10,this.center().y+this.scale*40));
    }
	else
	{
